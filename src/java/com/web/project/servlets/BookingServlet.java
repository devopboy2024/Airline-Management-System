...
public class BookingServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            */
            String flightid=request.getParameter("flight-id");
            int id=Integer.parseInt(flightid);
            String flightclass=request.getParameter("flight-class");
//            out.println("<h1>Servlet BookingServlet at " + request.getContextPath() + "</h1>");
            if(flightid==null){
            out.println("<h2> your id is null</h2>");
        }
            else{
              try{
                UserDao dao=new UserDao(ConnectionProvider.getconnection());
                AddFlight dd=dao.TicketBooking(flightclass,id);
                if(dd!=null){
//                    out.println(dd.getDepaturedate());
                HttpSession session=request.getSession();
                session.setAttribute("booking", dd);
                response.sendRedirect("TicketProfile.jsp");
                }
                else{
